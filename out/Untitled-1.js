...
class ClassAnalyzer {
    ...
    async findPotentialChildClasses(parentClassName) {
        const result = [];
...
class ClassHierarchyProvider {
    ...
    getChildren(element) {
        ...
        if (classNode.customTags.length > 0) {
            const tagsSection = new TagsSectionItem("Custom Tags", vscode.TreeItemCollapsibleState.Expanded);
        ...
        if (element instanceof TagsSectionItem) {
            ...
            if (parentNode) {
                return parentNode.customTags.map(tag => new TagItem(tag.tag.replace(/<c-/, '').replace(/>/, ''), tag.context, tag.line, parentNode.filePath, vscode.TreeItemCollapsibleState.None));
            }
        }
        return items;
    }
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('classHierarchy.selectMainClass', async () => {
        // Default folder name - use configuration or default to LWC
        const config = vscode.workspace.getConfiguration('classHierarchy');
        let folderName = config.get('defaultFolder') || "LWC";
    ...
    context.subscriptions.push(vscode.commands.registerCommand('classHierarchy.findAllCustomTags', async () => {
        vscode.window.withProgress({
            location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(vscode.commands.registerCommand('classHierarchy.openFile', (filePath) => {
        vscode.workspace.openTextDocument(filePath).then(doc => {
            vscode.window.showTextDocument(doc);
        });
    }));
    // Command to open a file at a specific line
    context.subscriptions.push(vscode.commands.registerCommand('classHierarchy.openFileAtLine', (filePath, line) => {
        vscode.workspace.openTextDocument(filePath).then(doc => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('classHierarchy.setAsMainClass', async () => {
