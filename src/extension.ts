...
class ClassNode {
    name: string; //LWC Name - com primeira letra minuscula
    filePath: string; //Path completo até o LWC
    children: ClassNode[] = [];
    // Track custom tags related to this class
	customTags: { tag: string, line: number, context: string, componentName: string, eventName?: string[], attributes?: string[] }[] = [];
...
class CustomEvent {
	name: string;
	eventBody :string;
	constructor(name: string, eventBody: string){
		this.name = name;
...
class ClassAnalyzer {
    ...
    private async findPotentialChildComponent(parentClassName: string): Promise<{name: string, filePath: string}[]> {
            ...
            if (fs.existsSync(htmlFilePath)) {
				...
				if (tagMatches) {
					for(const tagMatched of tagMatches){
						...
						classNode.children.push(new ClassNode(
							firstPart + remainingParts, 
							this.workspaceRoot+this.lwcFolderPath+firstPartUpperCase+remainingParts+'\\'+firstPart + remainingParts+'.html')
						);
					}
	...
	private async getCustomEvents(classNode: ClassNode): Promise<void>{
		try{
	...
	private async getPublicVariables(classNode: ClassNode, componentName: string): Promise<void>{
		try {
			const dirPath = path.dirname(classNode.filePath);
            const baseName = path.basename(classNode.filePath).split('.')[0];
			...
			if (fs.existsSync(htmlFilePath)) {
				...
				for (let i = 0; i < lines.length; i++) {
					...
					if(publicVariableMatch){
						classNode.publicVariables.push(
							new PublicVariable(publicVariableMatch[0], i)
						);
					}
				}
			}
    ...
    getChildren(element?: HierarchyItem): HierarchyItem[] {
		...
		if(element instanceof ClassHierarchyItem){
			if(classNode.publicVariables.length > 0){
				var publicVar = new PublicVariableItem(
					'@api Variables',
					classNode.publicVariables.length > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
				);
			...
			if(classNode.children.length > 0){
				const childNode = new ChildComponentItem(
					'Child Components',
				...
				for(const customTag of classNode.customTags){
					if(customTag.attributes != null && customTag.attributes?.length > 0){
						...
						const tagSection = new TagsSectionItem(
							"Child Params - " + customTag.componentName,
							classNode.customTags.length > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None,
							customTag.tag
						);
				...
				if(tagElement.attributes?.length != null &&  tagElement.attributes?.length >0){
					for(const attbItem of tagElement.attributes){
						...
						const tagItem = new TagItem(
							attbItem,attbItem,1,attbItem,vscode.TreeItemCollapsibleState.None
						);
						items.push(tagItem);
					}
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('lwcHierarchy.selectMainClass', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('lwcHierarchy.findAllCustomTags', async () => {
            /* vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('lwcHierarchy.openFile', (filePath: string) => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('lwcHierarchy.openFileAtLine', (filePath: string, line: number) => {
            /* vscode.workspace.openTextDocument(filePath).then(doc => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('lwcHierarchy.setAsMainClass', async () => {
